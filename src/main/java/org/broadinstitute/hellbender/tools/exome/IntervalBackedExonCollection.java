...
public final class IntervalBackedExonCollection implements ExonCollection<SimpleInterval> {
    ...
    /**
     * Comparator use to sort and search intervals.
     * <p>
     * Since we are guaranteed not to have overlaps between intervals (checked at construction),
     * we only need to pay attention to the contig (sorted alphanumerically) and the start position.
     * </p>
     */
    private Comparator<SimpleInterval> INTERVAL_COMPARATOR = (a,b) -> {
        if (a == null) {
            throw new IllegalArgumentException("the left element cannot be null");
        }
        if (b == null ) {
            throw new IllegalArgumentException("the right element cannot be null");
        }
        final int contigComparison = a.getContig().compareTo(b.getContig());
    ...
    public IntervalBackedExonCollection(final List<SimpleInterval> intervals) {
        ...
        sortedIntervals = intervals.stream().sorted(INTERVAL_COMPARATOR).collect(Collectors.toList());
    ...
    private int uncachedBinarySearch(final SimpleInterval location) {
        ...
        final int searchResult = Collections.binarySearch(sortedIntervals, location, INTERVAL_COMPARATOR);
        if (searchResult >= 0) {
            return searchResult;
        } else {
